...
def otsu_helper((otsufld, inn, size, otsu_factor)):
    ...
    #save input
    f = h5.File(os.path.join(otsufld, '{}_img.h5'.format(os.path.basename(inn)[:-4])), 'w')
    f['/main'] = raw
    f.close()
    
    #save input
    otsu = otsu_dilate(raw, lbl, size=size, otsu_factor=otsu_factor).astype('float32')
    f = h5.File(os.path.join(otsufld, '{}_lbl.h5'.format(os.path.basename(inn)[:-4])), 'w')
    f['/main'] = otsu
    f.close()    
    print inn
    return
...
if __name__ == "__main__":
    ...
    idx = 6
    arr = load_np(listdirfull(saveLocation, 'npy')[idx])
    plt.imshow(arr[0,25])
